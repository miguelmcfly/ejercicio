...
$('.btn-aceptprospecto').off('click').on('click',function(){
  let idprospecto = $(this).data('idprospecto');

  $.ajax({
    url: "/evaluacion/accept_prospecto",
    type: "POST",
    data: "idprospecto="+idprospecto,
    success: function(data) {
      console.log(data);
...
$('.btn-rechprospecto').off('click').on('click',function(){

  let idprospecto = $(this).data('idprospecto');

  $('#ModalRechazaProspecto_idprospecto').val(idprospecto);
  $('#ModalRechazaProspecto_motivo').html('');

  $('#ModalRechazaProspecto').modal('show');
